...
class Projects(commands.Cog, name="Projects"):
    ...
    @projects.command()
    async def create(
        self, ctx, name: str, owner: discord.Member = None
    ) -> discord.Message:
        ...
        if ctx.projects.find_project(name):
            ...
            if ctx.guild.get_Channel(int(project.get("channel"))):
                ...
                await ctx.send(
                    "A project with this name exists but, a related"
        ...
        if not ctx.bot.db("guilds").find(str(ctx.guild.id)).get("project_category"):
            overwrites = {
            ...
            category = await ctx.guild.create_category(
                "Flux Projects", overwrites=overwrites
            )
            ctx.bot.db("guilds").update(
                str(ctx.guild.id), {"project_category": str(category.id)}
            )

        else:
            category = ctx.guild.get_channel(
                int(
                    ctx.bot.db("guilds").find(str(ctx.guild.id)).get("project_category")
                )
            )
        ...
        channel = await ctx.guild.create_text_channel(
            f"{name}-project", category=category, overwrites=overwrites
        )
        ...
        if not res:
            return await ctx.send(
                "An error has occurred. Use `.contact`"
    ...
    async def delete(self, ctx, project_name: str) -> None:
            ...
            if channel and channel.category.name == "Flux Projects":
                if ctx.author.permissions_in(channel).manage_channels:
                    message = await ctx.send(
                        "That project doesn't appear to"
                    ...
                    reaction, user = await ctx.bot.wait_for(
                        ...
                        and (str(reaction.emoji) == yes or no)
                    ...
                    await ctx.send(
                        "That project does not appear to be in my "
        ...
        if str(ctx.author.id) != ctx.projects.find_project(project_name).get("owner"):
            await ctx.send("Only the project owner " "can delete this project.")
            return
        message = await ctx.send(
            "This action __cannot__ be undone. "
        ...
        if reaction.emoji.id == ctx.bot.config.tick_yes:
            channel = ctx.projects.find_project(project_name).get("channel")
    ...
    @projects.command()
    async def add(
        self, ctx, project_name: str, members: commands.Greedy[discord.Member]
    ) -> None:
    ...
    async def on_project_member_add(
        self, guild_id: int, project: dict, members: list
    ) -> discord.Message:
        ...
        if count == 1:
            ...
            return await channel.send(
                f"**> Member Update:** `{member}` was" " added to this project."
            )
        if count == 2:
            return await channel.send(
            ...
            members = members + f" and `{last_member}`"
            return await channel.send(
                f"**> Member Update:** {members} were " "added to this project."
            )
